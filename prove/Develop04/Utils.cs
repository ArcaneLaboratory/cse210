...
public class Utils
{
    ...
    /// <summary>
    /// Displays a command-prompt like interface via <c>Utils.GenericPrompt()</c> with a prompt on one line, followed by a "> " on the next.
    /// Waits for user input and returns true or false based on their response; true if their response contains the requested response, otherwise false; 
    /// </summary>
    /// <param name="prompt">the prompt to display to the user</param>
    /// <param name="response">the value to check the user's response against</param>
    /// <returns></returns>
    public static bool PromptOrEnter(string prompt, string response)
    {
        return GenericPrompt(prompt).ToLower().Contains(response.ToLower());
    }
